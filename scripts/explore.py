...
# code from https://github.com/mechwiz/jackal_exploration
...
class JackalExplore(object):
    def __init__(self):
        ...
        self.global_sub = rospy.Subscriber('/move_base/global_costmap/costmap', OccupancyGrid, self.cost_cb,
                                           queue_size=1)
        self.globalup_sub = rospy.Subscriber('/move_base/global_costmap/costmap_updates', OccupancyGridUpdate,
                                             self.costup_cb, queue_size=1)
        self.result_sub = rospy.Subscriber('move_base/result', MoveBaseActionResult, self.send_nav_cb)
        self.feedback_sub = rospy.Subscriber('move_base/feedback', MoveBaseActionFeedback, self.feedback_cb)
        self.poly_sub = rospy.Subscriber('/clicked_point', PointStamped, self.poly_cb, queue_size=1)
    ...
    def poly_cb(self, data):
        ...
        if self.polynum > 3:
            ...
            if calc_distance([x, y], closepnt) < 1:
                ...
                self.polydone = True
                self.send = True
                self.send_goal(self.currentpos)

        if not self.polydone:
    ...
    def send_nav_cb(self, data):
        ...
        if result.status.text == "Goal reached.":
            if self.flagg == 1 and self.init == 1:
                ...
                frontier_pnts = find_frontier(pic_mat)

                if self.polydone and len(frontier_pnts) > 0:
                    frontier_filter = []
                    rospy.loginfo('filter occuring')
                    for j in range(len(frontier_pnts)):
                        fx = mw(frontier_pnts[j][0])
                        fy = mh(frontier_pnts[j][1])
                        if self.polypath.contains_point([fx, fy]):
                ...
                if len(frontier_pnts) > 0:
                    ...
                    if self.send == True:
                        ...
                        self.send_goal(self.nextpnt)
                        self.isstuck = False
                else:
                    rospy.loginfo('Mapping Done')

    def send_goal(self, nextpnt):
